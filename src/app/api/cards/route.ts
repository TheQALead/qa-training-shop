...
export async function POST(request: NextRequest) {
  try {
    ...
    // БАГ КАРТЫ 1: Проверка номера с дублированной последней цифрой
    // Frontend дублирует последнюю цифру: 1234 -> 12344
    if (isBuggy) {
      // Проверяем, есть ли дублирование последней цифры
      const lastTwoDigits = number.slice(-2);
      if (lastTwoDigits[0] === lastTwoDigits[1]) {
        // Возвращаем ошибку с ОРФОГРАФИЧЕСКОЙ ОШИБКОЙ "даные"
        return NextResponse.json(
          { detail: 'Не верно указаны даные' },  // Орфографическая ошибка!
          { status: 400 }
        );
      }
    }
    ...
    const card = await db.card.create({
      data: {
        userId,
        owner,
        number: isBuggy ? number : number,
        date,
        cvv,
        balance: 20000, // Баланс по умолчанию
