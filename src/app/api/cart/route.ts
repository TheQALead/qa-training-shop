...
export async function GET(request: NextRequest) {
  try {
    ...
    // БАГ ПОДСЧЁТА СУММЫ (Server Side)
    let buggySum = realSum;
    if (realSum % 10 === 0) {
      // Если целое число десятков - добавляем 1000
    ...
    return NextResponse.json({
      ...
      totalItems,
      realSum,
      buggySum, // Frontend должен показывать эту сумму!
      sum: buggySum, // Алиас для совместимости
    });
  } catch (error) {
